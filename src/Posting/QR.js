...
var QR = {
    ...
    return $.queueTask(function () {
      if (!QR.inBubble()) {
        QR.hasFocus =
          ...
          QR.nodes.el.contains(document.activeElement);
  ...
  inBubble() {
    ...
    return (
      bubbles.includes(document.activeElement) ||
      bubbles.some(
        (el) =>
  ...
  blur() {
    if (QR.nodes.el.contains(document.activeElement)) {
  ...
  error(err, focusOverride) {
    ...
    if (!Header.areNotificationsEnabled) {
      if (document.hidden && !QR.cooldown.auto) {
  ...
  handleFiles(files) {
    ...
    if (
      document.activeElement === QR.nodes.fileButton &&
  ...
  response() {
    ...
    if (err) {
      ...
      QR.captcha.setup(
        QR.cooldown.auto &&
          [QR.nodes.status, document.body].includes(document.activeElement)
      );
      QR.status();
      QR.error(err);
      return;
