...
var v = JSON.parse(
  child_process.execSync('git show stable:version.json').toString()
);
var secrets = JSON.parse(
  fs.readFileSync(`../${pkg.meta.path}.keys/chrome-store.json`, 'utf8')
);
var refresh = JSON.parse(
  fs.readFileSync(`../${pkg.meta.path}.keys/refresh-token.json`, 'utf8')
);
...
import('chrome-webstore-upload').then((chromeWebstoreUpload) => {
  ...
  request(
    `https://chrome.google.com/webstore/detail/${pkg.meta.chromeStoreID}`,
    function (error, response, body) {
      if (
        body &&
        body.indexOf(`<meta itemprop="version" content="${v.version}"/>`) > 0 &&
